...
import { getScript, getTxBuilder, getBlockchainProvider } from '@/lib/contract_datum';
...
interface LockData {
  ...
  ownerHashes: string[];  // ← Added: saved owner hashes
  threshold: number;
  amount: string;
  scriptAddr: string;      // ← Added: saved script address
  timestamp: number;
}
...
export default function UnlockFunds({ wallet }: UnlockFundsProps) {
  ...
  useEffect(() => {
    const locks = JSON.parse(localStorage.getItem('multisigLocksParam') || '[]');
  ...
  const selectLock = async (lock: LockData) => {
    ...
    try {
      // Rebuild the contract using saved owner hashes
  ...
  const startSigning = async () => {
    ...
    try {
      // Rebuild contract with saved owner hashes
      const { scriptAddr, scriptCbor } = getScript(
        selectedLock.ownerHashes[0],
        selectedLock.ownerHashes[1],
        selectedLock.ownerHashes[2]
      );

      const scriptUtxo = utxoInfo;

      // Owner hashes are already saved in the lock data
      const ownerHashes = selectedLock.ownerHashes;
      ...
      // Add required signers (only first `threshold` owners)
  ...
  const submitTransaction = async (signedTx: string) => {
    ...
    try {
      ...
      if (selectedLock) {
        ...
        localStorage.setItem('multisigLocksParam', JSON.stringify(updatedLocks));
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-2xl font-bold text-white mb-2">Unlock Funds (Parameterized)</h3>
        <p className="text-purple-200/60">
          Collect signatures from owners (hardcoded in contract)
        </p>
      </div>
      ...
      <div>
        ...
        {availableLocks.length === 0 ? (
          <div className="backdrop-blur-lg bg-white/5 border border-white/10 rounded-xl p-6 text-center">
            <p className="text-purple-200/60 text-sm">No parameterized locks found</p>
            <p className="text-purple-300/50 text-xs mt-2">Lock funds using the parameterized contract first</p>
          </div>
        ) : (
